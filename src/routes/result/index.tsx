            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription className="space-x-2">
                        <em>/ˈfiːd.bæk/</em>
                        <span>
                            information given to guide future behavior, improve
                            performance, or enhance understanding, provided by
                            various sources like teachers, peers, or
                            experiences.
                        </span>
                    </CardDescription>
                </CardHeader>
                <CardContent className="grid lg:grid-cols-2 grid-cols-1 lg:gap-10 gap-8">
                    {competenciesWithIncidactors.map(
                        (competenctWithIndicators) => (
                            <CompetencyResult
    ...
    return (
        ...
        >
            ...
            <div className="grow">
                ...
                >
                    {props.competencyWithIndicators.indicators.map(
                        (indicator) => (
                            <CompetencyIndicator
                                key={indicator.name}
                                indicator={indicator}
...
function CompetencyIndicator(props: { indicator: Indicator }) {
    const feedback = useFeedbackStore(
        useShallow((store) =>
            store.feedback.find(
                (feedback) =>
                    feedback.metaData.indicator === props.indicator.name,
            ),
        ),
    );
